  ...
  try {
    ...
    if (!data.address) {
      return res
        ...
        .send({
          status: false,
          msg: "Address must contain street, city and pincode",
        });
    }
    if (!isValid(street)) {
      return res
    ...
    if (!isValidPinCode(pincode)) {
      return res
        .status(400)
        .send({ status: false, msg: "Pincode must contain 6 digits only" });
    }
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!email)
      return res
        .status(400)
        .send({ status: false, message: "Please provide email to login" });
    if (!password)
      return res
        .status(400)
        .send({ message: "Please provide password to login" });
